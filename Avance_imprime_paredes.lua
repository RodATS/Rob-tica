...
function actualizarDummys_SegunParedes(indices_Paredes)
    posPartX = 0
    posPartY = 0
    posPartTheta = 0
    if (indices_Paredes[1] == 8 and indices_Paredes[2] == 3 and indices_Paredes[3] == 2) then
        posPartX = -0.02
        posPartY = 0.02
        
    elseif (indices_Paredes[1] == 3 and indices_Paredes[2] == 1 and indices_Paredes[3] == 5) then
        posPartX = 0.02
        posPartY = 0.02
    end
...
function calculateLikelihood(x, y, theta, z)
    ...
    for indice, wall in ipairs(walls) do
        Ax = wall[1]
        Ay = wall[2]
        Bx = wall[3]
        By = wall[4]
        ...
        if (distanceToWall < m and distanceToWall >= 0) then
            ...
            if (isPointOnLineBetweenTwoPoints(intersectX, intersectY, Ax, Ay, Bx, By)) then
                m = distanceToWall
                print("Esta la pared", indice, " cerca")
